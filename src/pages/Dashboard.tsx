...
export default function Dashboard() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.log("Trace X: Initiating TraceX Research Engine...");
  ...
  const downloadReport = () => {
    ...
    doc.setFontSize(22);
    doc.text('TRACEX MISSION PROFILE', 20, currentY);
    
    currentY += 15;
    doc.setTextColor(150, 150, 150);
    doc.setFontSize(8);
    doc.text(`GENERATED BY: TRACEX INTELLIGENCE // OPERATOR: ${user?.email || 'N/A'}`, 20, currentY);
    doc.text(`TIMESTAMP: ${new Date().toLocaleString()} // STATUS: CLASSIFIED`, pageWidth - 20, currentY, { align: 'right' });

    currentY += 15;
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(12);
    doc.text(`TARGET ID: ${result.value} (${result.type.toUpperCase()})`, 20, currentY);
    
    currentY += 10;
    doc.setFontSize(10);
    doc.setTextColor(0, 255, 255);
    doc.text(`TLP: ${result.report.tlp}`, 20, currentY);
    doc.text(`CONFIDENCE: ${result.report.confidenceScore}%`, 80, currentY);
    doc.text(`THREAT LEVEL: ${result.report.threatLevel}`, 140, currentY);
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14);
    doc.text('EXECUTIVE BRIEF', 20, currentY);
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14);
    doc.text('TECHNICAL MISSION DATA', 20, currentY);
    
    currentY += 10;
    doc.setTextColor(180, 180, 180);
    doc.setFontSize(9);
    const splitReport = doc.splitTextToSize(result.report.report.replace(/[#*]/g, ''), 170);
    
    // Check if report fits on page, else add new page
    if (currentY + (splitReport.length * 5) > 270) {
      doc.text(splitReport.slice(0, Math.floor((270 - currentY) / 5)), 20, currentY);
      doc.addPage();
      doc.setFillColor(5, 5, 5);
      doc.rect(0, 0, 210, 300, 'F');
      currentY = 30;
      doc.text(splitReport.slice(Math.floor((270 - 30) / 5)), 20, currentY);
      currentY += (splitReport.slice(Math.floor((270 - 30) / 5)).length * 5) + 15;
    } else {
      doc.text(splitReport, 20, currentY);
      currentY += (splitReport.length * 5) + 15;
    }

    // Defensive Directives
    doc.setTextColor(0, 255, 255);
    doc.setFontSize(14);
    doc.text('DEFENSIVE DIRECTIVES', 20, currentY);
    currentY += 10;
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(10);
    
    (result.report.recommendations || []).forEach((rec: string, index: number) => {
    ...
    doc.text('--- END OF REPORT [TRACEX SECURE PIPELINE] ---', pageWidth / 2, 285, { align: 'center' });
    
    doc.save(`TRACEX-INTEL-${result.value.replace(/[^a-z0-9]/gi, '_')}.pdf`);
  };
        ...
        <aside className="w-72 border-r border-white/5 bg-black/40 flex flex-col hidden xl:flex shrink-0">
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold tracking-tighter text-sm uppercase">Trace X</span>
            </div>
            ...
            <div className="max-w-6xl mx-auto relative space-y-10">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="text-center md:text-left">
                  <h1 className="text-4xl font-black tracking-tighter uppercase text-white leading-tight">TraceX Intelligence</h1>
